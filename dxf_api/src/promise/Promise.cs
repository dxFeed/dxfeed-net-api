...
namespace com.dxfeed.promise
{
     ...
     * those two forms of invocation. If {@code findSomething} throws exception, then {@link #await() await} will
     * wrap them into {@link PromiseException}. If {@code findSomething} consults some kind of local cache with subscription,
     ...
     *
     * <pre>
     *     public Promise&lt;T&gt; findSomethingPromise(final Args args) {
     *         final Promise&lt;T&gt; promise = new {@link #Promise() Promise}&lt;T&gt;();
     *         executor.{@link Executor#execute(Runnable) execute}(new Runnable() {
     *             public void run() {
     *                 try {
     ...
     *     public Promise&lt;T&gt; findSomethingPromise(final Args args) {
     *         final Promise&lt;T&gt; promise = new {@link #Promise() Promise}&lt;T&gt;();
     *         final Future&lt;?&gt; future = executor.{@link ExecutorService#submit(Runnable) submit}(new Runnable() {
     *             public void run() {
     *                 try {
     *                     T result = findSomething(args);
     *                     promise.{@link #complete(Object) complete}(result);
     ...
     *             }
     *         });
     *         promise.whenDone(new {@link PromiseHandler}() {
     *             public void {@link PromiseHandler#promiseDone(Promise) promiseDone}(Promise&lt;? extends T&gt; promise) {
     *                 future.{@link Future#cancel(bool) cancel}(true); // true to interrupt running task
     *             }
     *         });
     *         return promise;
     *     }</pre>
     ...
     *
     * <pre>
     *     findSomething(args).whenDone(new {@link PromiseHandler}() {
     *         public void {@link PromiseHandler#promiseDone(Promise) promiseDone}(Promise&lt;? extends T&gt; promise) {
     ...
     * <h3>Design note</h3>
     *
     * This class is based on the {@link Future} and {@code CompletableFuture} (since Java 8). However, the methods
    ...
    public class Promise<T> where T : class
    {
        ...
        private PromiseHandler<T> handler;

        private CancellationTokenSource cancelTokenSource = null;
        private TaskCompletionSource<T> completionSource = null;
        ...
        {
            completionSource = new TaskCompletionSource<T>(TaskStatus.RanToCompletion);
            completionSource.SetResult(result);
        }

        private Promise(Exception exception)
        {
            if (exception == null)
                throw new NullReferenceException();
            completionSource = new TaskCompletionSource<T>(TaskStatus.Faulted);
        ...
        public bool HasResult
        {
            get
            {
                return completionSource.Task.Status == TaskStatus.RanToCompletion;
            }
        }
        ...
        public bool HasException
        {
            get
            {
                return completionSource.Task.IsCanceled || completionSource.Task.IsFaulted;
            }
        }
        ...
        public bool IsCancelled
        {
            get
            {
                return completionSource.Task.IsCanceled;
            }
        }
        ...
        {
            return completionSource.Task.Exception;
        }
        ...
        {
            cancelTokenSource = timeout.Equals(TimeSpan.Zero) ? new CancellationTokenSource() : new CancellationTokenSource(timeout);
            Task<T> task = Task.Run<T>(() => TaskFunction(cancelTokenSource.Token), cancelTokenSource.Token);
            try
            {
	     ...
	     * @throws PromiseException if computation has completed exceptionally.
	     */
        public bool AwaitWithoutException(TimeSpan timeout)
        {
            try
            {
                Await(timeout);
        ...
        {
            if (completionSource.Task.AsyncState != null && (TaskStatus)completionSource.Task.AsyncState != TaskStatus.Running)
                return;
            completionSource.SetResult(result);
            handleDone(handler);
        }
        ...
        //	     */
        //        public void WhenDone(PromiseHandler<T> handler)
        //        {
        //            if (handler == null)
        //                throw new NullReferenceException();
        //            bool done;
        ...
        //                    else if (this.handler is Handlers<T>)
        //                        (this.handler as Handlers<T>).add(handler);
        //                    else
        ...
        //	     */
        //        public void whenDoneAsync(PromiseHandler<T> handler/*, Executor executor*/)
        //        {
        ...
        //	     */
        //	    public static Promise<T> completed(T result)
        //{
         ...
         */
        protected void handleDone(PromiseHandler<T> handler)
        {
            if (handler != null)
                handler.PromiseDone(this);
        }
        ...
        //TODO: comments
        protected T TaskFunction(CancellationToken cancelToken)
        {
            return null;
        }
        ...
        //private bool awaitImpl(TimeSpan timeout)
        //{
        ...
        //private class Handlers<T> : PromiseHandler<T> where T : class
        //{
        //    List<PromiseHandler<T>> handlers = new List<PromiseHandler<T>>();

        //    public Handlers(params PromiseHandler<T>[] handlers)
        //    {
        //        this.handlers.AddRange(handlers);
        //    }

        //    public void add(PromiseHandler<T> handler)
        //    {
        //        handlers.Add(handler);
        //    }


        //    public void PromiseDone(Promise<T> promise)
        //    {
        //        foreach (PromiseHandler<T> handler in handlers)
