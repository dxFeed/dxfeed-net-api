...
namespace com.dxfeed.native
{
    ...
    public class OrderViewSubscription : IDxSubscription, IDxOrderListener
    {
        ...
        {
            if (subscription == null) throw new InvalidOperationException(AddSymbolBeforeSourceErrorText);
        ...
        private void RemoveSymbolSourcePair(string symbolSource)
        {
            ...
            foreach (var key in keysList) snapshots.Remove(key);
        ...
        /// <summary>
        ///     <para>
        ///         Removes multiply symbols from subscription.
        ///     </para>
        ///     <para>
        ...
        public void RemoveSymbols(params string[] symbols)
        {
            ...
            foreach (var symbol in symbols)
            {
                ...
                foreach (var source in sources) RemoveSymbolSourcePair(upperSymbol + source);
        ...
        /// <summary>
        ///     Sets symbols to subscription.
        ///     <remarks>
        ///         Do not set more than one symbol. Otherwise, this API mechanism is unstable.
        ///     </remarks>
        /// </summary>
        /// <param name="symbols">List of symbols.</param>
        /// <exception cref="ArgumentException">Invalid <paramref name="symbols" /> parameter.</exception>
        ...
        /// <summary>
        ///     Adds <see cref="OrderSource" /> to subscription.
        ///     <remarks>
        ///         In fact, it sets the sources.
        ///         Do not set more than one source. Otherwise, this API mechanism is unstable.
        ///     </remarks>
        /// </summary>
        ...
        /// <summary>
        ///     Removes existing <see cref="OrderSource" /> from subscription and set new.
        ///     <remarks>
        ///         Do not set more than one source. Otherwise, this API mechanism is unstable.
        ///     </remarks>
        /// </summary>
        ...
        {
            if (connection == null) throw new InvalidOperationException("Object is disposed");
            ...
            foreach (var source in sources) this.sources.Add(OrderSource.ValueOf(source));
        }
            ...
            where TE : IDxOrder
        {
            ...
            if (snapshots.ContainsKey(key))
            {
                ...
                if (buf.EventParams.Flags.HasFlag(EventFlag.SnapshotBegin))
                {
                    ...
                    foreach (var order in buf) receivedSnapshots[symbol].Remove(order.Source);
                    ...
                    !buf.EventParams.Flags.HasFlag(EventFlag.SnapshotEnd))
                {
                        ...
                        orderViewStates[symbol].HasFlag(OrderViewState.Pending))
                    {
                            ...
                            orderViewStates[symbol].HasFlag(OrderViewState.Pending))
                        {
                            ...
                            foreach (var order in snapshots[key]) buffer.Add(order);
                    ...
                    {
                        var buffer = new OrderEventBuffer(buf.EventType, buf.Symbol, buf.EventParams);
                        foreach (var order in buf) buffer.Add(ZeroOrder(order));
                ...
                // no flags no actions
                if (buf.EventParams.Flags == 0) return;
                    ...
                    !buf.EventParams.Flags.HasFlag(EventFlag.RemoveEvent))
                {
                    ...
                    foreach (var order in buf) snapshots[key].Add(order);
                }
            ...
            else
            {
                ...
                if (buf.EventParams.Flags.HasFlag(EventFlag.SnapshotEnd)) SnapshotEndFlagReceived<TB, TE>(buf);
            }
        }
            ...
            where TE : IDxOrder
        {
            ...
            foreach (var receivedSource in receivedSnapshots[symbol])
            {
                ...
                {
                    foreach (var order in snapshots[snapshotKey]) resultBuffer.Add(order);
